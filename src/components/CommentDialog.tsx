...
function CommentDialog({ interviewId }: { interviewId: Id<'interviews'> }) {
  ...
  const handleSubmit = async () => {
    if (!comment.trim()) return toast.error('댓글을 입력하세요')
          ...
          {existingComments.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-medium">이전 댓글</h4>
                <Badge variant="outline">{existingComments.length} 댓글</Badge>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="후보자에 대한 자세한 의견을 공유하세요..."
                className="h-32"
              />
            </div>
          </div>
