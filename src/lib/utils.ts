...
export const calculateRecordingDuration = (
  ...
  if (duration.hours && duration.hours > 0) {
    return `${duration.hours}시간 ${String(duration.minutes).padStart(
      2,
      '0'
    )}분 ${String(duration.seconds).padStart(2, '0')}초`
  }

  if (duration.minutes && duration.minutes > 0) {
    return `${duration.minutes}분 ${String(duration.seconds).padStart(
      2,
      '0'
    )}초`
  }
